...
class Metadata {
  ...
  /**
   * Shortcut for unwrapping a metadata field that is a path as an absolute absolute path
   * or null when it is missing.
   *
   * @param path Path to make absolute.
   * @returns Absolute path as a string, or null when missing.
   */
  private def get_absolute_path(String path) {
    if (path == null) return null

    def file_path = Paths.get(path)
    ...
    return metadata_dir.resolve(file_path).toString()
  }

  /**
   * Shortcut for making sure that the file at the given path exists and is readable.
   *
   * @param field_key  The key of the metadata field which the file path was extracted from.
   *                   Is only used to improve error messages when the assertion fails.
  ...
  /**
   * Makes sure that all the columns names in the metadata file actually exists in the header of
   * the sumstat file.
   *
   * @param header     The sumstat header to validate.
   * @throws Exception When a column name is missing from the sumstat header.
   */
  public def validate_sumstat_header(SumstatHeader header) {
    Metadata.class.metaClass.getProperties().each { prop ->
  ...
  /**
   * Retrieves all fields that starts with the given prefix in a map.
   *
   * @param prefix The prefix that the fields must match.
   * @returns Map of fields.
   */
  public def fields_by_prefix(String prefix) {
    def fields = [:]

    Metadata.class.metaClass.getProperties().each { prop ->
      if (!prop.name.startsWith(prefix)) return
