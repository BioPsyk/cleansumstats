...
awk -vFS="\t" -vOFS="\t" 'NR>=1{for(i=9; i<=NF; i++){$i=$i*$8}; printf "%s%s%s%s%s%s%s%s%s%s", $1,OFS, $4,OFS, $5,OFS, $6,OFS, $7,OFS; for(i=9; i<NF; i++){printf "%s%s", $i, OFS}; print $NF}' | LC_ALL=C join -t "$(printf '\t')" -1 1 -2 1 - <(sstools-utils ad-hoc-do -f $STAT -k "0|${var_k}" -n"0,${nam_k}" )
