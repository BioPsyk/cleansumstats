...
singularity pull ibp-cleansumstats-base_version-1.0.3.simg docker://biopsyk/ibp-cleansumstats:1.0.3
mkdir -p tmp
chmod ug+rwX tmp
...
There is no support for relative links in the metadata file, which means all files have to be in the same folder. However, you can provide paths with associated files `-p "path/to/folder1:path/to/folder2`
```
