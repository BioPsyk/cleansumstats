...
work_dir="${project_dir}/tmp/regression-282"
outdir="${work_dir}/out"
...
echo "regression-282-started"

# Redirect all output to log file
exec > "${log_dir}/regression-282.log" 2>&1

rm -rf "${work_dir}"
mkdir "${work_dir}"

echo ">> Test regression #282"
...
cat <<EOF > ./input.txt
SNP CHR BP A1 A2 FREQ_A1 EFFECT_A1 SE P	DIRECTION metaN
rs6439928	chr3	141663261	T	C	0.658	-0.0157	0.0141	0.2648	+-?	3
rs6443624	chr3	180380376	A	C	0.242	0.0027	0.0155	0.8603	--?	3
rs6444089	chr3	187159052	A	G	0.55	0.0043	0.0128	0.7394	+-?	3
rs645184	chr11	73806781	A	G	0.608	0.003	0.0129	0.8169	+-?	3
rs645510	chr12	116569153	T	C	0.675	-0.0151	0.0143	0.292	+-+	3
rs6456063	chr6	166624019	A	G	0.783	0.0245	0.018	0.1735	+-+	3
rs6458154	chr6	40461984	A	G	0.458	0.0126	0.0132	0.3407	+-+	3
EOF

cat <<EOF > ./expected-result1.tsv
CHR	POS	0	RSID	EffectAllele	OtherAllele	B	SE	Z	P	EAF_1KG	Direction	StudyN
12	117668628	5	rs645510	C	T	0.0151	0.0143	1.055944	5.10505e-01	0.33	+-+	3
3	140461721	1	rs6439928	T	C	-0.0157	0.0141	-1.113475	5.43501e-01	0.68	+-?	3
EOF
...
then
  cat .nextflow.log
  echo "regression-282-failed" > /dev/stderr
  exit 1
fi
...
echo "regression-282-succeeded" > /dev/stderr
